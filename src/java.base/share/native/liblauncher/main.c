...
char* getProgName(char *path) {
    ...
    if (filename == NULL) {
        filename = path; // No path separator found, use the whole path
    } else {
        filename++; // Move past the path separator
    }

    // Duplicate the filename to avoid modifying the original string
    ...
    // Find the last '.' to remove the file extension
    char *dot = strrchr(filenameDup, '.');
    if (dot != NULL) {
        *dot = '\0'; // Remove the extension by truncating the string
    }

    return filenameDup;
}

char* loadJavaArgs(char* progName) {
    char jimagePath[MAXPATHLEN];
    ...
    if (!GetJImageFilePath(jimagePath, sizeof(jimagePath))) {
        JLI_TraceLauncher("Cannot locate the jimage file\n");
        return NULL;
    }

    JImageFile *jimage = JIMAGE_Open(jimagePath, &err);
    JLI_Snprintf(resource, sizeof(resource), "jdk/internal/misc/resources/%s", progName);
    loc = JIMAGE_FindResource(jimage, "java.base", "9.0", resource, &size);
    buf = JLI_MemAlloc((size_t) size);
    JIMAGE_GetResource(jimage, loc, buf, size);

    JLI_TraceLauncher("Load java argument: %s\n", buf);
    return buf;
}
